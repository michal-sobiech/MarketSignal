...
builder.Services.AddDbContext<MarketDbContext>((serviceProvider, options) => {
    ...
    string connectionString = $"Server={marketDbOptions.Host};Port={marketDbOptions.Port};Database={marketDbOptions.DbName};User Id={marketDbOptions.UserName};Password={marketDbOptions.Password};AllowPublicKeyRetrieval=True;";
...
builder.Services.AddSingleton<RedisOptions>(_ => new RedisOptions {
    ...
    Port = int.Parse(Environment.GetEnvironmentVariable("REDIS_HOST") ?? throw new ArgumentException("REDIS_PORT is not set"))
});
